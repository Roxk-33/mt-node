...
module.exports = app => {
	...
	router.get('/v1/user/address/:id', verfiyToken, controller.user.getAddressDetail); //-获取用户收货地址详情
	...
	router.delete('/v1/user/address/:id', verfiyToken, controller.user.delAddress);
	// 评价
	router.get('/v1/user/evaluation/', verfiyToken, controller.user.getEvalList);
	router.delete('/v1/user/evaluation/:id', verfiyToken, controller.user.deleteEval);
	...
	// 清空某商店的购物车
	router.delete('/v1/user/cart/:shopId', verfiyToken, controller.cartList.empty);

	// 去结算-入口为商店内，结算商品是该商店购物车全部商品
	// 去结算-入口为购物车，结算商品是该商店购物车部分（也有可能是全部）商品
	// 具体展示那些商品由前端处理
	router.get('/v1/user/cart/settle/:shopId', verfiyToken, controller.cartList.getListByShop);

	// 创建订单
	router.post('/v1/order', verfiyToken, controller.order.create);
	router.put('/v1/order/pay', verfiyToken, controller.order.payOrder);
	router.get('/v1/order/pay/:id', verfiyToken, controller.order.getOrderPayInfo);
	...
	// 评价
	router.post('/v1/user/order/:id/review', verfiyToken, controller.order.review);
};
